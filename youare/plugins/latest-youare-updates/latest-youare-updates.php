...
Version: 0.2
Description:  Get your latest updates from YouAre. To show off them just put <code>&lt;?php if (function_exists('get_youare_latest_updates')) get_youare_latest_updates(); ?&gt;</code>  in your template. When you activate this plugin, visit <a href="options-general.php?page=latest-youare-updates/latest-youare-updates.php">options</a> to insert your username
Author: YouAre Team
...
<table class="form-table">
 ...
 <tr>
    ...
    <td>
      <textarea id="youare_html_template" name="youare_html_template"><?php $yo_ht_te = get_option('youare_html_template'); echo $yo_ht_te?$yo_ht_te:'<p>%content% %permalink%</p>'?></textarea>
...
function get_youare_latest_updates() {
  ...
  if (!$cache || $latest_time) {
    ...
    $updates = @file_get_contents('http://youare.com/api/updates/'.get_option('youare_username').'/php');
    if (!$updates) {
      echo __('There was a problem reading your latest YouAre updates', 'latest-youare-updates');
      return;
    }
    $updates = unserialize($updates);
